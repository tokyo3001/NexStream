...
function Modal() {
  const [showModal, setShowMoal] = useRecoilState(modalState) 
          ...
          <div className="absolute bottom-10 flex w-full items-center justify-between px-10">
            <div className="flex space-x-2">
                ...
                font-bold text-black transition hover:bg-[#e6e6e6]">
                 <FaPlay className="h- w-7 text-black"/>
                 Play</button>
            ...
            <div className="space-y-6 text-lg">
                <div className="flex items-center space-x-2 text-xs">
                    <p className="font-semibold text-green-400">
                        {movie!.vote_average * 10}% Match</p>
                    <p className="font-light">
                        {movie?.release_date || movie?.first_air_date}</p>
                    <div className="flex h-4 item-center justify-center rounded border border-white/40
                    px-1.5 text-xs">
                        HD
                    </div>
                </div>
